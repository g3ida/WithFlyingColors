...
// so a wall the level was built out of and a slab of it that moves read as the same thing.
//
// The run itself is PlatformSlide, shared with every other moving surface in the game, so a moving
// wall waits, travels, crushes and is put back by a checkpoint exactly like a flat platform does.
[Tool]
[ScenePath]
public partial class SlidingBreakerBrickPlatform : BreakerBrickPlatform, IPersistent {
  ...
  // disabled: what the cube stands on is the boxes built off the painted bricks, and what a platform
  ...
  public override string[] _GetConfigurationWarnings() {
    ...
    if (Mathf.IsZeroApprox(Distance)) {
      warnings.Add("Distance is zero, so the platform stays where it is. Use a plain BreakerBrickPlatform for a wall that does not move.");
    }
    if (Speed <= 0.0f) {
  ...
  #region Checkpoints
  // Only the run is saved. What the wall has lost is not: a respawn lays every brick back, so there
  // is nothing about the bricks left to remember.
  ...
  private void _onSlideCheckpointReached(Vector2 _position, string _colorGroup) => _slide.OnCheckpointReached();

  private void _onSlideRespawn() {
    _slide.Reset();
    SetPhysicsProcess(true);
  }
